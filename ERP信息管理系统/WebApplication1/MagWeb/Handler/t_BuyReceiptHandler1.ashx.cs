...
{
    /// <summary>
    /// t_BuyReceiptHandler1 的摘要说明
    /// </summary>
    public class t_BuyReceiptHandler1 : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (orderid == "" || begin == "" || end == "")
            {
                ...
                ",\"rows\":" + JsonConvert.SerializeObject(t_BuyReceiptManages.GetBuyReceipt(index, count, orderid, begin, end, receorderid)) + "}");
            }
            else if (orderid != "" && begin != "" && end != "")
            {
                context.Response.Write("{\"total\":" + t_BuyReceiptManages.Getcount() +
            ",\"rows\":" + JsonConvert.SerializeObject(t_BuyReceiptManages.GetBuyAReceipt(index, count, orderid, begin, end, receorderid)) + "}");
            } 
        }
